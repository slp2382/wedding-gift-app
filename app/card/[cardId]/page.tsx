...
export default function CardPage() {
  ...
  async function handleClaim() {
    if (!card || card.claimed || !cardId) return;
    ...
    if (error) {
      ...
      setErrorMessage(error.message || "Could not mark this gift as claimed.");
    } else {
      setCard(data as CardRecord);
    }
  ...
  const isClaimed = card?.claimed ?? false;
  ...
  return (
    <div className="min-h-screen bg-zinc-50 text-zinc-900 px-4 py-10 flex items-center justify-center dark:bg-zinc-950 dark:text-zinc-50">
      <main className="w-full max-w-xl space-y-8">
        ...
        {/* Loading state */}
        {loading && (
          <section className="rounded-2xl border border-zinc-200 bg-white p-6 text-center shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        ...
        {!loading && errorMessage && (
          <section className="rounded-2xl border border-rose-200 bg-rose-50 p-6 text-center shadow-sm dark:border-rose-800 dark:bg-rose-950/40">
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-lg font-semibold">
                    {card.giver_name || "A generous guest"}
                  </p>
                </div>
                ...
                {isClaimed ? (
                  ...
                  <>
                    <p className="text-sm text-zinc-600 dark:text-zinc-300">
                      ...
                      this card. In a later version, this will also transfer the
                      funds.
                    </p>
                    <button
                      onClick={handleClaim}
